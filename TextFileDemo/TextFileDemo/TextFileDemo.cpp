...
void writeTextFile()
{
	...
	ofstream bookTextFile("books.txt", ios::out);	//because opened at out file deletes previous. Could use ate= add to end
...
void readTextFile()
{
	...
	while ( !bookTextFile.eof() )	//eof= end of file
	{
		// Get inputs from file.
		getline(bookTextFile, theTitle, '\t');		// scans from the start of line to the first tab
		getline(bookTextFile, theAuthor, '\t');		// continues to scan and ends at next tab		
		bookTextFile >> theYear;	// 	Conversions: stoi( theIntString.c_str() )	stoi=string to int
									//				 stod( theDoubleString.c_str() ) stod = string to double
